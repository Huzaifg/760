...
class QLearning(AbstractSolver):
    ...
    def train_episode(self):
        ...
        for t in range(self.options.steps):
            action_probs = self.epsilon_greedy_action(state) # Get action using epsilon greedy policy
            action = np.random.choice(np.arange(len(action_probs)), p=action_probs) # Choose action epsilon-greedily
            next_state, reward, done, _ = self.step(action) # Take a step
            self.Q[state][action] +=  self.options.alpha * (reward + self.options.gamma * np.max(self.Q[next_state]) - self.Q[state][action]) # Update Q
            if done: 
                break
